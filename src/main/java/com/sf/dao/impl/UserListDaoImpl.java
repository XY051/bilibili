...
public class UserListDaoImpl implements UserListDao {
	...
	public List<videoEntity> videolist(String videocAtegory) {
		//将全部视频查询出来
		String sql="select * from video where videocAtegory=? && ispass=1 && ischeck=1 order by rand() limit 8";//将视频标记为1的正常视频全部查询出
	...
	public List<videoEntity> videolistimit6MAD() {
		String sql="select * from video where ispass=1 && ischeck=1 order by rand() limit 6";
		List<videoEntity> list=jdbcTemplate.getJdbcTemplate().query(sql,new Object[]{} ,new VideoRowMapperEntity());		
	...
	@Override
	public List<userEntity> userlistUserid(String user_id) {
		//根据名字查询出用户对象. - -
		String sql="select * from user where userID=?";
		List<userEntity> user=jdbcTemplate.getJdbcTemplate().query(sql,new Object[]{user_id},new RowMapperEntity());
		return user;
	}

	@Override
	public List<videoEntity> videolistimi8MAD(int videotype) {
		String sql="select * from video where ispass=1 && ischeck=1 && videocAtegory=? order by rand() limit 8";
		List<videoEntity> list=jdbcTemplate.getJdbcTemplate().query(sql,new Object[]{new String(videotype+"")} ,new VideoRowMapperEntity());
